...
CMake’s `project()` command accepts a `VERSION` argument made of numeric components like `major.minor.patch[.tweak]`, and it sets project version variables such as `PROJECT_VERSION`, `PROJECT_VERSION_MAJOR`, `PROJECT_VERSION_MINOR`, `PROJECT_VERSION_PATCH`, and `PROJECT_VERSION_TWEAK`. :contentReference[oaicite:0]{index=0}
---
## Why this matters
